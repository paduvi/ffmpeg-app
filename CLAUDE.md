...
- `resources/` — static assets. `models/resnet18_identity.onnx`, `icon.{icns,ico,png}`, `sample-images/` (permanent built-in samples seeded to SQLite on first launch), and `img/convert-icon.png` (the one surviving JavaFX-era icon, mirrored into `src/renderer/public/img/`). **Only `models/`, `sample-images/` and `icon.png` are actually packaged** — `electron-builder.yml` excludes `resources/**` wholesale and re-adds those three via `extraResources`, so anything else here is repo-only. A further 16 JavaFX-era icons sat unreferenced until they were removed; check for a real consumer before adding files here.
...
- `electron-builder.yml` — packaging config. Targets: mac dmg+pkg+zip, win nsis+msi+zip (the Windows zip is the **portable** build — the `win-unpacked` tree archived with no wrapping folder, and with `isWriteUpdateInfo=false` in electron-builder, so it never enters `latest.yml` and cannot compete with nsis for `electron-updater`). The **MSI is `perMachine: true`** — set under a top-level `msi:` block, which `MsiTarget` deep-assigns over `win:`, so the NSIS one-click installer keeps its own per-user behaviour. That writes `ALLUSERS=1` into the package, and Windows then asks for elevation once, up front, before anything is written; the electron-builder default is per-user (`ALLUSERS=2` + `MSIINSTALLPERUSER=1`), which installs silently into `%LOCALAPPDATA%` for one user with no UAC prompt at all. **No arch arrays in the config** — arch comes from CLI flags (`--arm64`/`--x64`), one per invocation: config arch overrides the CLI and makes a single run build both arches, which races the pkg target on a shared intermediate file and packages host-arch sharp into the foreign-arch app. `publish: github` so `electron-updater` finds release feeds.
...
Push a tag `vX.Y.Z`; build jobs upload artifacts and a dedicated release job publishes DMG/PKG/ZIP/EXE/MSI plus the `latest*.yml` update feeds and `.blockmap` files to a GitHub Release (no PAT needed — the release job grants `contents: write` to the default token). The two mac architectures are **not** merged: Apple Silicon ships the default `latest-mac.yml`; the release job renames the Intel feed to `latest-x64-mac.yml`, and Intel installs request it via `autoUpdater.channel = 'latest-x64'` in `updater.ts`. The mac `zip` target exists because electron-updater on macOS cannot update from a dmg; the Windows `zip` is unrelated — it is the portable build, and the release job's `*.zip` glob publishes both. The build action asserts all three Windows artifacts (`.exe`, `.msi`, `*-win.zip`) exist before uploading, since `if-no-files-found: error` only fires when *nothing* matched and would let a silently dropped target through. Manual run: `workflow_dispatch` with an optional version input (builds artifacts, no release). Installed apps auto-check on launch (production only — gated on `app.isPackaged`). On Windows the update downloads in the background and shows a "Restart Now / Later" dialog; on macOS (unsigned) it cannot self-install, so it opens the releases page for a manual download instead (see the macOS auto-update note under "Things not obvious").
...
- **Output directory timestamps use local time**, not UTC. Matches the legacy app. `makeOutputDir` lives in `services/outputDir.ts` and is shared by compression and cutting — both once held a verbatim copy, which is precisely how that invariant would have drifted.
